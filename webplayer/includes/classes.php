	...
	function getReport($id) {
		...
		if($row) {
			...
			if($row['type'] == 0) {
				...
				$content = '<div class="report-content"><img src="'.$this->url.'/thumb.php?src='.$result['art'].'&t=m&w=112&h=112" height="15" width="15"> <a href="'.permalink($this->url.'/index.php?a=track&id='.$result['id'].'&name='.cleanUrl($result['title'])).'" target="_blank">'.$result['title'].'</a> '.mb_strtolower($LNG['by']).' <img src="'.$this->url.'/thumb.php?src='.$result['image'].'&t=a&w=50&h=50" height="15" width="15" /> <a href="'.$this->url.'/index.php?a=admin&b=users&id='.$result['idu'].'" target="_blank">'.realName($result['username']).'</a></div>';
			}
...
class feed {
	...
	function playlistArt($id, $limit) {
		...
		if(!empty($rows)) {
			...
			$output .= '<div style="transform:rotate(0deg); -webkit-transform:rotate(0deg); -ms-transform:rotate(0deg); position: absolute; float: left; z-index: 0"><img src="'.$this->url.'/thumb.php?src=default.png&h=100&w=100&t=m" id="playlist-art'.$id.'"></div>';
		}
		return $output;
	}
		...
		if($result->num_rows) {
			...
			if($result->num_rows) {
				...
				if($type == 1) {					
					...
					$stmt = $this->db->prepare(sprintf("INSERT INTO `reports` (`track`, `parent`, `content`, `by`, `type`, `reason`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')", $this->db->real_escape_string($id), (isset($parent['tid'])) ? $parent['tid'] : 0, $this->db->real_escape_string(htmlspecialchars(trim(nl2clean($reason)))), $this->db->real_escape_string($this->id), $this->db->real_escape_string($type), $reason));
				} else {
